...
- `verify.mjs` — `cases/*.input.json` 을 읽어 `selectUrls` + `mergeFetchResults` 를 실행하고 `cases/*.expected.json` 과 비교한다.
- `cases/case-*.input.json` / `cases/case-*.expected.json` — 케이스별 입력/기대 출력.

각 케이스의 입력은 `{ policy: {...}, fetchResults: [{ url, status, cleanedText, extraAttachments }] }` 형태이며, 기대 출력은 `{ selectedUrls: [...], merged: { cleanedText, extraAttachments, status } }` 형태다.
