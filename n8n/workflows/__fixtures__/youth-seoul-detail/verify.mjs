...
// ---------------------------------------------------------------------------
// regionFromDistrictTitle 단위 검증 (괄호 (○○구) 우선 → 제목 토큰 정확매칭 → 폴백 서울특별시)
// ---------------------------------------------------------------------------
const regionUnitCases = [
  // 제목 끝 (구명) → 해당 구
  ['2026년 미취업청년 자격증 응시료 지원사업(중랑구)', '중랑구'],
  ['청년 월세 지원(강남구)', '강남구'],
  // 괄호 없이 제목 토큰에 구명 → 해당 구 (실제 적재 케이스)
  ['2026년 중랑구 청년 재테크 교육', '중랑구'],
  ['강남구 청년 월세 지원 사업', '강남구'],
  ['서울 중구 청년 일자리사업', '중구'],
  // 제목에 구명 없음 → 본청/전체 정책 = 서울특별시
  ['2026년 서울청년정책네트워크 하반기 모집', '서울특별시'],
  ['2026 서울 동아리ON(대학생 동아리 지원)', '서울특별시'],
  // 자치구명 부분문자열은 정확매칭이라 오인 안 함 (진로 ≠ 종로구)
  ['청년 진로 탐색 지원 사업', '서울특별시'],
  // 비공백 인접 구명은 토큰 분리가 안 돼 폴백(의도된 한계 — 오분류보다 안전)
  ['강남구청년센터 청년 모집', '서울특별시'],
  // footer 주소에 "중구" 가 있어도 우편번호/"(우)" 시그널로 토큰 탐색 생략 → 서울특별시
